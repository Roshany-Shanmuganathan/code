var jvx_685e3f84c33d4NonMraidTagInMraidEnv = false;
var jvx_685e3f84c33d4dyPropsACK = false;

	var jvx_685e3f84c33d4omidSupported = null, jvx_685e3f84c33d4isLayoutLoaded = false;
	var jvx_685e3f84c33d4OmidImprMessageTimer = null;
	var jvx_685e3f84c33d4OmidImprMessageHandler = null;
	var jvx_685e3f84c33d4OmidPostMsgAck = false;
	var jvx_685e3f84c33d4OmidImprRaised = false;
(function(){
	if(typeof MRAID_ENV != 'undefined'){
		...
		jvx_685e3f84c33d4NonMraidTagInMraidEnv = true;
	}
})();

if(typeof jvxObjectList == "undefined"){
	var jvxObjectList = ["jvx_685e3f84c33d4"];
}else{
	jvxObjectList.push("jvx_685e3f84c33d4");
}
...
function createiFrameNode(obj){
	...
	if(document.body == null){
		document.write("<div id='jvxSkeletonRef_685e3f84c33d4' style='display:none;'></div>")
	}
			...
			createImprTrackerFrame = function(frameId){
				...
				function jvx_685e3f84c33d4fireImprTracker(url, elType){
			var elNode = null;
			if(elType == 'script'){
							} else {
...
finalURL = replaceCacheBuster4;				//finalURL = replaceCacheBuster;
			...
			if(elType == 'script'){
														imprScrTrackerCnt++;
					var imprFrameId = 'jvx_685e3f84c33d4ImprTrackerFrame_'+imprScrTrackerCnt;
			...
			function jvx_685e3f84c33d4fireIT(){
																						jvx_685e3f84c33d4fireImprTracker('https://ad.doubleclick.net/ddm/trackimpj/N997352.5402186DV360_SAMEA_PT/B32360969.400771417;dc_trk_aid=593715626;dc_trk_cid=220578289;ord=1751008604;dc_lat=;dc_rdid=;tag_for_child_directed_treatment=;tfua=;gdpr=0;gdpr_consent=;ltd=;dc_tdv=1;dc_dbm_token=AOjeLNEAAABpCmEKDAgAFQAAAAAdAAAAABIMCAAVAAAAAB0AAAAAIhQI_JH51lKoAr6k3tEDsAKaiafmA0AB0gIqGAAiEwjG-tPc_5COAxXo-3MBHaAgGV4oATABOKLsn-X_EkACSAJYiIEgENO-8poCo-UdOdA_k7pJRO1xpuJp2w?', 'script');
																				}
...
function replaceRunTimeMacros( trackerUrl, params){
	try{
		...
		runtimeMacroValues['ah'] = (jvx_685e3f84c33d4.baseHeight) ? jvx_685e3f84c33d4.baseHeight : (params['clientHeight'] ? params['clientHeight'] : "") ;
		runtimeMacroValues['aw'] =  jvx_685e3f84c33d4.baseWidth ? jvx_685e3f84c33d4.baseWidth : (params['clientWidth'] ? params['clientWidth'] : "");
		var tv = runtimeMacroValues['ct'] - jvx_685e3f84c33d4.layoutLoadedTime;
...
*/
var jvx_685e3f84c33d4TraffickedInFrame = false,			// COM: should be true, if !busted and self != top
	jvx_685e3f84c33d4SafeFrameSupported = false,		// COM: should be true only when safeFrame is supported in the page
	jvx_685e3f84c33d4CanBustFromFrame = true,			// COM: should be false, when all busting method fails
	jvx_685e3f84c33d4InNestedFrame = false;				// COM: should be true, when tag trafficked inside nested frame

if(self != top){
	jvx_685e3f84c33d4TraffickedInFrame = true;
}

try{
		$sf.ext.register(160, 600);
		jvx_685e3f84c33d4SafeFrameSupported = true;
}catch(e){
	jvx_685e3f84c33d4SafeFrameSupported = false;
}
...
var adViewabilityDetectInit = function (visibilityThreshold) { 
    try {
        ...
        boxElement = document.querySelector('#adUnitContainer_685e3f84c33d4');
                    ...
                    if (!adInVPForOneSec && !timerRunning) {
                        ...
                        timeoutHandle = setTimeout(function () {
                            adInVPForOneSec = true;
                                                        if(typeof jvx_685e3f84c33d4 != "undefined")
                            jvx_685e3f84c33d4.recordEvent(73);
                            else 
                            recordEvent(73);
                                                    }, 1000);
                    }
...
var CreativeUnit = (function(){
	...
	function receiveHtml5Message(e){
        ...
        var s = e.data.split(':', 10);
		        if(s && s.length >= 2 && s[0] == 'jvx_685e3f84c33d4'){
			var msg = s[1];
			if(msg == 'openExpansion'){
									jvx_685e3f84c33d4.openClickThrough();
							} else if(msg == 'closeExpansion'){
				jvx_685e3f84c33d4.closeExpansion(false, false);
			} else if(msg == 'expansionLoaded'){
							} else if(msg == 'recordUIR'){
				jvx_685e3f84c33d4.UIRRecorded = true;
			} else if(msg == 'lastMouseLeave'){
				jvx_685e3f84c33d4.lastMLeaveId = s[2];
			} else if(msg == 'clearUIR'){
				clearTimeout(jvx_685e3f84c33d4.UIRTimer);
			} else if(msg == 'recordDwell'){
				if(!jvx_685e3f84c33d4.DwellRecorded){
					jvx_685e3f84c33d4.recordDwellEvent(49, jvx_685e3f84c33d4.DwellElapsedTime);
				}
			} 
						 else if(msg == 'clearOmidMsgHandler'){
				jvx_685e3f84c33d4OmidPostMsgAck = true;
				window.clearInterval(jvx_685e3f84c33d4OmidImprMessageHandler);
				window.clearTimeout(jvx_685e3f84c33d4OmidImprMessageTimer);
			}
												else if(msg=="dynamicProperties") {
                jvx_685e3f84c33d4.updateDYProperties(s[2],1);
							}
			else if(msg=="clickThroughUrl") { 
							
				jvx_685e3f84c33d4.clickTagURL = s.slice(2).flat(1).join(":");
			}
			else if(msg == "clickTrackers") {	
								jvx_685e3f84c33d4.clickTrackers =  s.slice(2).join(":").split(",").map(function(url){ return decodeURIComponent(url) });
				this.clickTrackers = jvx_685e3f84c33d4.clickTrackers;
			}
												                        else if(msg == "miniScrollerBgInfo"){
                var miniScrollerBgURL = decodeURIComponent(s[2]);
                jvx_685e3f84c33d4.fixedBgAstLeft = parseInt(s[3]);
                jvx_685e3f84c33d4.renderFixedPageBg(miniScrollerBgURL,s[3]);
            } 
		}
	}
	...
	function postHtml5Message(winRef, msg){
		try{
			winRef.postMessage('jvx_685e3f84c33d4' + ':' + msg, "*");
		}catch(e){
			if(typeof(winRef.contentWindow) != 'undefined')
				winRef.contentWindow.postMessage('jvx_685e3f84c33d4' + ':' + msg, "*");
		}
	}
	...
	*/
	...
	function clearNode(pNode, idPrefix){
        ...
        if(jvx_685e3f84c33d4TraffickedInFrame){
                        try{
...
finalURL = replaceCacheBuster4;			//finalURL = replaceCacheBuster;
			...
			if(url.indexOf("?") == -1){
				...
				finalURL = url + "&r=" + Math.random();
					...
					if(typeof jvx_685e3f84c33d4 != 'undefined' && url.match("https://evs.jivox.com/jivox/serverAPIs/saveImpression.php")){	//Add es_cgName and es_segName params only for the evs trackers URL not for the thirdParty trackers.
				finalURL += "&es_cgName_"+jvx_685e3f84c33d4.DYReportingKey;
                if(jvx_685e3f84c33d4.DYSegmentName != ""){
                    finalURL += "&es_segName_"+jvx_685e3f84c33d4.DYSegmentName;
                }
			}
				imgEl.src = finalURL;
			}       
	...
	function setOpacity(myElement, opacityValue) {
		...
		var cLeft = sfLeft + "px",
			cTop = sfTop + "px",
			cNode = getElById(jvx_685e3f84c33d4.adUnitContainerId);
		addStyles(cNode, {"left":cLeft, "top":cTop});
	}
	function renderAdInsideSafeFrame(animDir){
		if(jvx_685e3f84c33d4SafeFrameSupported){
			/* ------------- SafeFrame Settings--------------- */
						var ref = jvx_685e3f84c33d4,
	 ...
	 /* var rect = obj.getBoundingClientRect();
		...
		var vpStyles = findPos(getElById(jvx_685e3f84c33d4.adUnitContainerId)),
			vpTop = "", vpLeft = "";
	...
	function mraidHandleOnViewable(viewable){
		jvxMraidIsShown = viewable;		 
			if(!jvxMraidIsShown || getElById(jvx_685e3f84c33d4.baseContainerId)){
				return; 
			}
		jvx_685e3f84c33d4.init();
	...
	function handleMraidViewableImprFiring(){
		if (!adInVPForOneSec && !timerRunning) {
			...
			var timeoutHandle = setTimeout(function () {
				adInVPForOneSec = true;
				jvx_685e3f84c33d4.recordEvent(74);
			}, 1000);
		}
	}
	...
	function CreativeUnit(uniqId){
		...
		this.browserVersion = '138.0.0.0';
								this.clickTagURL = 'https://googleads.g.doubleclick.net/dbm/clk?sa=L&ai=CekFjWD1eaMbuM-j3z7sPoMHk8AXf6_KpfqLsn-X_Esur8-X1ChABII69nhxgkansBKABveLD3ijIAQmpAngfOaRJGYE-qAMByAObBKoEpQJP0MnnrkqRrrtWEIRBBI0NNxLuz7OGc_yqscsOmrqbzhHwIsXtvNbH56QHLun88W7RqwF0UBX_vnAGTNsoOFvIbVQUyq4EzxNVwPddTLZwUHOz55Kjecn7ZjLWDESfBj1T7XuN1miXarq29CcSnevOMXQRFR8RSFRItyMWI5ELyMiQDSKvyDhAtS0__wNc02vG44D3eESctuKapXfl2fx9xhvc2WaVzOOiBhVK4zbs_fpQD63YrWOmlyQUww_2z7_X1tl97QWKVY4MeKclN28PSnx9OoOYwc1I19U6rPrIUpoVTTvxGn936fFMxlRFjNEIMKg5lI59AjPbAXLs6twxVz2UNwEUoK-mITWF4TY6H9j1Z2zm7Lhv0l0FPDd-waISsbDA3cAE87bPhJUF4AQDiAX8kfnWUpAGAaAGTYAHvZqUvgOIBwGQBwKoB6a-G6gHzM6xAqgH89EbqAeW2BuoB6qbsQKoB47OG6gHk9gbqAfw4BuoB-6WsQKoB_6esQKoB6--sQKoB9XJG6gH2baxAqgHmgaoB_-esQKoB9-fsQKoB_jCsQKoB_vCsQKoB-fXsQLYBwDSCCwIABACGJ0BMgEAOhKf0ICAgIAEgMCAgICgqIACqAFIvf3BOlijx8_c_5COA_IIG2FkeC1zdWJzeW4tNjQxODE5ODcwMjU1NjkzN4AKA5gLAcgLAYAMAaoNAk
E7ZvISDOJ37OPkj9YY5LUmuAesO_AZcqfA%26cry%3D1%26dbm_d%3DAKAmf-B3XUMP3wqUrtClu9wzriPa6tEPraCIcMqzKoThdqEwPx1wkQo1CxkUXdUqjptzEHz3i6txSY3w7YOxh8szleLk3DG6bgu_eFXqRt_-IRMD7lC58HhdHvLX86LAjj4wkPFN0ZUTfByz6Ja2Blynd1cRcdbvDtLg5Bne1qqN4OH4Wk4ItNAU_WrdQKfjG1aTltrtzLESVQXBvOQzvQDi7aBPmBEckhLQOy5slWnFDhh4R8aNMC2aB8pRPvDF5JhIMdZj9vqTkK0AlWirmZ2YUUWSNaJDnK7NMjk9902bZGassK1bErwaTDkLVCGRRCMsYQgKemipOKsyFq2YnKIHOwPEtXJcWo06GBuQcass2NtWeL8t6VijwZrbWKQ1rscNja-zHUIv34hMvQwk82k_J0EYqEVOjB4LscLVjdoHWQHzNhf0wBM1UDaykwll4QBImP0BnCIzElQvlXJmbVY1I87fKOENjfGELgdPAlVbZWEYPSYDR5xHsfdIp2ZivOEBSJdv7BHS9PTGiLOanAD5uk-FVFFu3a__NEZvCbTjZJvomcJyfgJdKp8U1Wu4i0FDFZkDfX_KNIRrePuyf6K0U9cDmbjwRZilzQ4C2_m4duUKUmTELKpWEr1qAqjX4XItPNmYcgL7REgO0QflAePoRTaamxG3BA%26adurl%3D&c_dv360_source_url=https%3A%2F%2Fwww.w3schools.com%2Fcss%2Fcss_boxmodel.asp&objectName=jvx_685e3f84c33d4&adUnitId=1900&jvxSessionId=1751007108.3908';
		this.layoutLoadedTime = 0;
        			this.baseURL += '&base=1';
            this.baseURL += '&creativeResolveBeginTime='+Date.parse(creativeResolveBeginTime);
            							this.baseURL += '&omid=1';
					        this.baseBgType = 'none';
		this.baseBgURL = 'https://assets.jivox.com/assets/widgets/2025/5/a76754z681a2ee3c6333/1/160x600_VI/HTML_M.html';
	...
	CreativeUnit.prototype = {
		constructor:CreativeUnit,
         ...
         sendCookieRequest: function(cookieName){
	        if(jvx_685e3f84c33d4SafeFrameSupported){
		         var callBackFnName ='jvx_685e3f84c33d4'+".updateDYProperties";
		         jvx_685e3f84c33d4.addScriptTagToBody('https://as.jivox.com'+"/unit/getCookies.php?cookieName="+cookieName+"&callback="+callBackFnName+"&deleteCookie="+1+"&r_="+Math.random());  
	         }                  
         },
         updateDYProperties: function(val,isNotCookieData){
            ...
            jvx_685e3f84c33d4.DYReportingKey = dynPropArr[0] ? decodeURIComponent(dynPropArr[0]) : "";
            jvx_685e3f84c33d4.DYSegmentName = dynPropArr[1] ? decodeURIComponent(dynPropArr[1]) : "";
		  jvx_685e3f84c33d4.eventServerParams = dynPropArr[2] ? decodeURIComponent(dynPropArr[2]) : "";   //Getting trafficker events output from layout_renderer_html.php
		  jvx_685e3f84c33d4.cmUrl = dynPropArr[3] ? decodeURIComponent(dynPropArr[3]) : "";
			jvx_685e3f84c33d4.reportingSchema = dynPropArr[4] ? decodeURIComponent(dynPropArr[4]) : "";
            //this.fireCREvent();
			jvx_685e3f84c33d4dyPropsACK = true;
         },
			...
			if(doFireCreativeLoadEvt){
				...
				jvx_685e3f84c33d4.recordEvent(72, 'es_et=' + creativeLoadElapseTime);
			}
		},
		onLoadEventHandler:function(event){
                    ...
                    if(jvx_685e3f84c33d4SafeFrameSupported){
                        var getReportingKeyFromCookie = false,
                            ...
                            if(getReportingKeyFromCookie){
                                jvx_685e3f84c33d4.sendCookieRequest('jvx_685e3f84c33d4'+"_dynamicProperties");
                            }
                    }
                											}	
		},
			...
			baseContainerNode.appendChild(baseNode);
									...
									jvx_685e3f84c33d4isLayoutLoaded = true;
				if(jvx_685e3f84c33d4omidSupported == true){
					postHtml5Message(getElById(that.baseId),"omidSupported:true");
				}
				else if(jvx_685e3f84c33d4omidSupported == false){
					postHtml5Message(getElById(that.baseId),"omidSupported:false");
				}
				if(!jvx_685e3f84c33d4OmidPostMsgAck && jvx_685e3f84c33d4OmidImprRaised){
                    postHtml5Message(getElById(that.baseId),"omidImpressionSignal:true");
                }
								jvx_685e3f84c33d4.layoutLoadedTime = Date.now();
			...
			var executeCallBack = function(transitionEvt){
					...
					if(ref.expansionOpen){
						...
						if(ref.openExpAgain) ref.openExpAgain = false; 
												...
												if(jvx_685e3f84c33d4SafeFrameSupported && transitionEvt){
							try{
								$sf.ext.collapse();
															}catch(e){}
						}
		...
		getIBusterFrameInfo:function(){
			if(typeof(jivoxIBusterAdObject) != "undefined"){
				...
				for(var i=0; i < iFramesLen; i++){
					if(hostIframes[i].contentWindow == jivoxIBusterAdObject["jvx_685e3f84c33d4"].foreignWindow){
...
//				e2Node.style.margin = mt + ' 0 0 ' + ml;
		...
		openWindow:function(url){
							if(jvx_685e3f84c33d4NonMraidTagInMraidEnv){
					mraid.open(url);
				} else {
					window.open(url);
				}
		...
		openClickThrough:function(event={}){
				...
				if(!this.raiseCMacroFlag){
					var eventRecordParams = "c_cx="+clickEventDetails.clientX+"/c_cy="+clickEventDetails.clientY+"/c_ct="+clickEventDetails.ct+"/c_tv="+(clickEventDetails.ct-jvx_685e3f84c33d4.layoutLoadedTime);
					this.recordEvent(1, eventRecordParams);
				}else{
					var eventRecordParams = "es_clkMac_1/c_cx="+clickEventDetails.clientX+"/c_cy="+clickEventDetails.clientY+"/c_ct="+clickEventDetails.ct+"/c_tv="+(clickEventDetails.ct-jvx_685e3f84c33d4.layoutLoadedTime);
					this.recordEvent(1, eventRecordParams);
				}
									if(!adInVPForOneSec){
												this.recordEvent(73);
		...
		recordEvent:function(eventId,addnlParams){
											if( !jvx_685e3f84c33d4dyPropsACK && (eventId==48 || eventId==49 || eventId==1)){
					addnlParams = (addnlParams) ? addnlParams + "/c_actualEventId="+eventId : "c_actualEventId="+eventId;
					eventId = 78;
				}
								var eventReportingParams = '';
											eventReportingParams = "229482/1900/181754/674dd6df7af248b/19/jvxSId_1751007108.3908";		
												var rand = Math.random();
				var imgEl = document.createElement("img");
                var tempURL = "";
                var eventReportingURL = "https://evs.jivox.com/trk" + "/" + eventId + "/" + eventReportingParams;
                eventReportingURL += "/es_pId_3d372812";
                tempURL += "";
                                tempURL += "/ap_DataSignal1=22194047228/ap_DataSignal3=AOjeLNEAAABpCmEKDAgAFQAAAAAdAAAAABIMCAAVAAAAAB0AAAAAIhQI_JH51lKoAr6k3tEDsAKaiafmA0AB0gIqGAAiEwjG-tPc_5COAxXo-3MBHaAgGV4oATABOKLsn-X_EkACSAJYiIEgENO-8poCo-UdOdA_k7pJRO1xpuJp2w/ap_DataSignal2=GBL-DV360";
                									if(jvx_685e3f84c33d4.cmUrl != "" && jvx_685e3f84c33d4.cmUrl != undefined){
						tempURL += "/cmUrl=" + encodeURIComponent(jvx_685e3f84c33d4.cmUrl);
					}
					if(jvx_685e3f84c33d4.eventServerParams != "" && jvx_685e3f84c33d4.eventServerParams != undefined){
						tempURL += "/" + jvx_685e3f84c33d4.eventServerParams;
					}
				                tempURL = addnlParams ? (tempURL + "/" + addnlParams) : tempURL;
									tempURL += "/bDim="+ "160x600";
								tempURL = tempURL + "/jvxRandom=" + rand;
				//eventReportingURL = (eventId == 1) ? (eventReportingURL + ";https://googleads.g.doubleclick.net/dbm/clk?sa=L&ai=CekFjWD1eaMbuM-j3z7sPoMHk8AXf6_KpfqLsn-X_Esur8-X1ChABII69nhxgkansBKABveLD3ijIAQmpAngfOaRJGYE-qAMByAObBKoEpQJP0MnnrkqRrrtWEIRBBI0NNxLuz7OGc_yqscsOmrqbzhHwIsXtvNbH56QHLun88W7RqwF0UBX_vnAGTNsoOFvIbVQUyq4EzxNVwPddTLZwUHOz55Kjecn7ZjLWDESfBj1T7XuN1miXarq29CcSnevOMXQRFR8RSFRItyMWI5ELyMiQDSKvyDhAtS0__wNc02vG44D3eESctuKapXfl2fx9xhvc2WaVzOOiBhVK4zbs_fpQD63YrWOmlyQUww_2z7_X1tl97QWKVY4MeKclN28PSnx9OoOYwc1I19U6rPrIUpoVTTvxGn936fFMxlRFjNEIMKg5lI59AjPbAXLs6twxVz2UNwEUoK-mITWF4TY6H9j1Z2zm7Lhv0l0FPDd-waISsbDA3cAE87bPhJUF4AQDiAX8kfnWUpAGAaAGTYAHvZqUvgOIBwGQBwKoB6a-G6gHzM6xAqgH89EbqAeW2BuoB6qbsQKoB47OG6gHk9gbqAfw4BuoB-6WsQKoB_6esQKoB6--sQKoB9XJG6gH2baxAqgHmgaoB_-esQKoB9-fsQKoB_jCsQKoB_vCsQKoB-fXsQLYBwDSCCwIABACGJ0BMgEAOhKf0ICAgIAEgMCAgICgqIACqAFIvf3BOlijx8_c_5COA_IIG2FkeC1zdWJzeW4tNjQxODE5ODcwMjU1NjkzN4AKA5gLAcgLAYAMAaoNAkxL4g0TCI7j0Nz_kI4DFej7cwEdoCAZXuoNEwjhwtLc_5COAxXo-3MBHaAgGV7wDQGwE7nJ-RzQEwDYEw3YFAHQFQHKFgIKAPgWAYAXAbIXNBgCKjAvMjIxNTI3MTgvc3dzLWhiLy93M3NjaG9vbHMuY29tLy93aWRlX3NreXNjcmFwZXKyGAkSAqBPGE0iAQA&ae=1&num=1&cid=CAQSVQDZpuyztmXr4EGYyOcahaKo1h13MOzuZPQhR3REkxTF5sfey3QbbTAWHQNM2bZyYPcBNV7tegicReW75IOug-4eFBYhA3v_zv57EgZWOIA0UZTzThIYAQ&sig=AOD64_0koH4SkMz3n4PZ4fmFwEXj0eWZVA&client=ca-pub-4276969157128104&dbm_c=AKAmf-BpfNZK19Ip3x3RFEpMp40dgS_h3120Vo0mg3nSe_MjEdN7A-y4ztqnLCxRkRvfl079CBLf2_vUexRHGkHU2PfZ95NYVwRDdNmKBlQznNggc-QI_uBOvPFJeQgfBGBZ3yKyVLh92VCtTAInWy5gAES-g5bX1EafOEPnTqjJYISdXMnBSjHtzojsomzOE7ZvISDOJ37OPkj9YY5LUmuAesO_AZcqfA&cry=1&dbm_d=AKAmf-B3XUMP3wqUrtClu9wzriPa6tEPraCIcMqzKoThdqEwPx1wkQo1CxkUXdUqjptzEHz3i6txSY3w7YOxh8szleLk3DG6bgu_eFXqRt_-IRMD7lC58HhdHvLX86LAjj4wkPFN0ZUTfByz6Ja2Blynd1cRcdbvDtLg5Bne1qqN4OH4Wk4ItNAU_WrdQKfjG1aTltrtzLESVQXBvOQzvQDi7aBPmBEckhLQOy5slWnFDhh4R8aNMC2aB8pRPvDF5JhIMdZj9vqTkK0AlWirmZ2YUUWSNaJDnK7NMjk9902bZGassK1bErwaTDkLVCGRRCMsYQgKemipOKsyFq2YnKIHOwPEtXJcWo06GBuQcass2NtWeL8t6VijwZrbWKQ1rscNja-zHUIv34hMvQwk82k_J0EYqEVOjB4LscLVjdoHWQHzNhf0wBM1UDaykwll4QBImP0BnCIzElQvlXJmbVY1I87fKOENjfGELgdPAlVbZWEYPSYDR5xHsfdIp2ZivOEBSJdv7BHS9PTGiLOanAD5uk-FVFFu3a__NEZvCbTjZJvomcJyfgJdKp8U1Wu4i0FDFZkDfX_KNIRrePuyf6K0U9cDmbjwRZilzQ4C2_m4duUKUmTELKpWEr1qAqjX4XItPNmYcgL7REgO0QflAePoRTaamxG3BA&adurl=") : eventReportingURL;
									tempURL += "/es_cgName="+jvx_685e3f84c33d4.DYReportingKey;
                    if(jvx_685e3f84c33d4.DYSegmentName != ""){
                        tempURL += "/es_segName="+jvx_685e3f84c33d4.DYSegmentName;
                    }
		...
		recordDwellEvent:function(eventId, elapsedTime){
			...
			this.DwellTimer = null;
						...
						var cNode = createDivNode(this.adUnitContainerId);
			            												document.write('<div id="jvxScrRefNode_685e3f84c33d4" style="display:none;"></div>');	
				var jvxPNode = '';
				var jvxNode = getElById("jvxScrRefNode_685e3f84c33d4");
			...
			if(bestFitBase) {
				...
				if(this.baseHasLayout == 1) {
					this.baseType = 'html';
					this.baseURL = 'https://as.jivox.com/unit/layout_renderer.php?dspId=996f2104-601b-4eda-b1f2-2d6660bf451e&campaignId=181754&showAdChoices=0&jvxVer=2&isDynamic=1&siteId=674dd6df7af248b&es_pId=3d372812&ts_pId=3d372812&creativeUnitType=19&bDim=160x600&bUnitId=1900&us_privacy=%24%7BUS_PRIVACY%7D&gdpr=0&gdpr_consent=&ap_DataSignal1=22194047228&c_Au
mKBlQznNggc-QI_uBOvPFJeQgfBGBZ3yKyVLh92VCtTAInWy5gAES-g5bX1EafOEPnTqjJYISdXMnBSjHtzojsomzOE7ZvISDOJ37OPkj9YY5LUmuAesO_AZcqfA%26cry%3D1%26dbm_d%3DAKAmf-B3XUMP3wqUrtClu9wzriPa6tEPraCIcMqzKoThdqEwPx1wkQo1CxkUXdUqjptzEHz3i6txSY3w7YOxh8szleLk3DG6bgu_eFXqRt_-IRMD7lC58HhdHvLX86LAjj4wkPFN0ZUTfByz6Ja2Blynd1cRcdbvDtLg5Bne1qqN4OH4Wk4ItNAU_WrdQKfjG1aTltrtzLESVQXBvOQzvQDi7aBPmBEckhLQOy5slWnFDhh4R8aNMC2aB8pRPvDF5JhIMdZj9vqTkK0AlWirmZ2YUUWSNaJDnK7NMjk9902bZGassK1bErwaTDkLVCGRRCMsYQgKemipOKsyFq2YnKIHOwPEtXJcWo06GBuQcass2NtWeL8t6VijwZrbWKQ1rscNja-zHUIv34hMvQwk82k_J0EYqEVOjB4LscLVjdoHWQHzNhf0wBM1UDaykwll4QBImP0BnCIzElQvlXJmbVY1I87fKOENjfGELgdPAlVbZWEYPSYDR5xHsfdIp2ZivOEBSJdv7BHS9PTGiLOanAD5uk-FVFFu3a__NEZvCbTjZJvomcJyfgJdKp8U1Wu4i0FDFZkDfX_KNIRrePuyf6K0U9cDmbjwRZilzQ4C2_m4duUKUmTELKpWEr1qAqjX4XItPNmYcgL7REgO0QflAePoRTaamxG3BA%26adurl%3D&c_dv360_source_url=https%3A%2F%2Fwww.w3schools.com%2Fcss%2Fcss_boxmodel.asp&objectName=jvx_685e3f84c33d4';
											this.baseURL += '&jvxSessionId=1751007108.3908';
			...
			if(bestFitExpand) {
				...
				if(this.expansionHasLayout == 1) {
					this.expansionType = 'html';
										var adUnitIdStr = '';
										this.expansionURL = 'https://as.jivox.com/unit/layout_renderer.php?dspId=996f2104-601b-4eda-b1f2-2d6660bf451e&campaignId=181754&showAdChoices=0&jvxVer=2&isDynamic=1&siteId=674dd6df7af248b&es_pId=3d372812&ts_pId=3d372812&creativeUnitType=19&bDim=160x600&&us_privacy=%24%7BUS_PRIVACY%7D&gdpr=0&gdpr_consent=&ap_DataSignal1=22194047228&c_Auctionid=ABAjH0i1VZJ1q96Rb_J6YQzuX5n4&ap_DataSignal3=AOjeLNEAAABpCmEKDAgAFQAAAAAdAAAAABIMCAAVAAAAAB0AAAAAIhQI_JH51lKoAr6k3tEDsAKaiafmA0AB0gIqGAAiEwjG-tPc_5COAxXo-3MBHaAgGV4oATABOKLsn-X_EkACSAJYiIEgENO-8poCo-UdOdA_k7pJRO1xpuJp2w&ap_DataSignal2=GBL-DV360&r=1751006552849734&cMacro=https%3A%2F%2Fgoogleads.g.doubleclick.net%2Fdbm%2Fclk%3Fsa%3DL%26ai%3DCekFjWD1eaMbuM-j3z7sPoMHk8AXf6_KpfqLsn-X_Esur8-X1ChABII69nhxgkansBKABveLD3ijIAQmpAngfOaRJGYE-qAMByAObBKoEpQJP0MnnrkqRr
SNaJDnK7NMjk9902bZGassK1bErwaTDkLVCGRRCMsYQgKemipOKsyFq2YnKIHOwPEtXJcWo06GBuQcass2NtWeL8t6VijwZrbWKQ1rscNja-zHUIv34hMvQwk82k_J0EYqEVOjB4LscLVjdoHWQHzNhf0wBM1UDaykwll4QBImP0BnCIzElQvlXJmbVY1I87fKOENjfGELgdPAlVbZWEYPSYDR5xHsfdIp2ZivOEBSJdv7BHS9PTGiLOanAD5uk-FVFFu3a__NEZvCbTjZJvomcJyfgJdKp8U1Wu4i0FDFZkDfX_KNIRrePuyf6K0U9cDmbjwRZilzQ4C2_m4duUKUmTELKpWEr1qAqjX4XItPNmYcgL7REgO0QflAePoRTaamxG3BA%26adurl%3D&c_dv360_source_url=https%3A%2F%2Fwww.w3schools.com%2Fcss%2Fcss_boxmodel.asp&objectName=jvx_685e3f84c33d4'+adUnitIdStr;
											this.expansionURL += '&jvxSessionId=1751007108.3908';
		...
		listenerCallback:function(event) {
			jvx_685e3f84c33d4.eventController(event); 
		},
...
try{
        ...
        omidScript.onload = function(){
            var iframeNode = jvx_685e3f84c33d4.getElById(jvx_685e3f84c33d4.baseId); 
            ...
            if (omidClient.isSupported()) {
                jvx_685e3f84c33d4omidSupported = true;
                if(jvx_685e3f84c33d4isLayoutLoaded){
                    jvx_685e3f84c33d4.sendPostMessage(iframeNode,"omidSupported:true");
                }
                jvx_685e3f84c33d4.recordEvent(90,'c_omidSupported=true');
                omidClient.registerSessionObserver(function(obj){
                    if(obj.type == 'sessionStart'){
                        jvx_685e3f84c33d4.recordEvent(91,'c_omidSessionStart='+encodeURIComponent(JSON.stringify(obj)));
                    }
                }); 
                omidClient.addEventListener('impression', function(obj){
                    jvx_685e3f84c33d4OmidImprRaised = true;
                    jvx_685e3f84c33d4.sendPostMessage(iframeNode,"omidImpressionSignal:true");
                    jvx_685e3f84c33d4OmidImprMessageHandler = setInterval(function () {
                        jvx_685e3f84c33d4.sendPostMessage(iframeNode,"omidImpressionSignal:true");
                    },300);
                    jvx_685e3f84c33d4OmidImprMessageTimer = setTimeout(function(){
                        window.clearInterval(jvx_685e3f84c33d4OmidImprMessageHandler);
                    },15000);
                    jvx_685e3f84c33d4.recordEvent(93,'c_omidImprReceived=1');
                                            jvx_685e3f84c33d4fireIT();
                                    }); 
            }
            else{
                jvx_685e3f84c33d4omidSupported = false;
                if(jvx_685e3f84c33d4isLayoutLoaded){
                    jvx_685e3f84c33d4.sendPostMessage(iframeNode,"omidSupported:false");
                }
                                    jvx_685e3f84c33d4fireIT();
                                jvx_685e3f84c33d4.recordEvent(92,"c_omidSupported=false");
            } 
        }
...
var jvx_685e3f84c33d4 = new CreativeUnit("_685e3f84c33d4");
jvx_685e3f84c33d4.DYReportingKey = "";
jvx_685e3f84c33d4.DYSegmentName = "";
jvx_685e3f84c33d4.eventServerParams = "";
jvx_685e3f84c33d4.cmUrl = "";
	if(jvx_685e3f84c33d4NonMraidTagInMraidEnv){
				jvx_685e3f84c33d4.recordEvent(75);
	}
    jvx_685e3f84c33d4.init();
